...
class Kompas(object):
    ...
    def open_document(self, file_path: str):
        ...
        """
        try:
            # Используем Documents.Open для открытия файла
            doc = self.Documents.Open(file_path)
            print(f"Документ открыт: {file_path}")
            return doc
    ...
    def save_document(self, file_path: str = None):
        ...
        :param file_path: путь для сохранения (если None - сохраняет с текущим именем)
        """
        doc = self.application.ActiveDocument
        if doc is None:
            print("Ошибка: Нет активного документа для сохранения")
            return
        try:
            if file_path:
                # Сохранить как
    ...
    def close_document(self, save_changes=True):
        """
        Закрывает активный документ
        :param save_changes: сохранять изменения перед закрытием (True/False)
        """
        doc = self.application.ActiveDocument
        if doc is None:
            print("Нет активного документа для закрытия")
            return False
        try:
    ...
    def set_litera_1(self, litera: str):
        """
        Устанавливает литеру в основной надписи в первую ячейку
        :param litera: литера
        """
        self._set_frame_field(col_num=40, text=litera.upper(), font_height=5, style=32771)
        print(f"Установлена литера: {litera.upper()}")

    def set_litera_2(self, litera: str):
        """
        Устанавливает литеру в основной надписи во вторую ячейку
        :param litera: литера
        """
        self._set_frame_field(col_num=41, text=litera.upper(), font_height=5, style=32771)
        print(f"Установлена литера: {litera.upper()}")

    def set_litera_3(self, litera: str):
        """
        Устанавливает литеру в основной надписи в третью ячейку
        :param litera: литера
        """
        self._set_frame_field(col_num=42, text=litera.upper(), font_height=5, style=32771)
        print(f"Установлена литера: {litera.upper()}")
